...
public class Fencer extends Navigator {
	...
	protected static BufferedImage[] blueImages = null;
	...
	public static BufferedImage[] getBallAndStick(Color color) {
		...
		/**
		 * Produces an overhead view of a person. The given color is the color of their
		 * shoulders. Images are as follows: 0: Idle, facing forward 1: Idle, sideways
		 * 2-6: sword coming out 7-11: sword parrying
		 **/
		// square stance
	...
	public ScaledImage draw() {
		if (images == null) {
			if (blueImages == null) {
				blueImages = getBallAndStick(Color.BLUE);
			}
			images = blueImages;
		}
		switch (action) {
		...
		default:
			if (intention == SolidActor.ATTACK) {
				/**
				 * If Player.intention == ATTACK, it will always appear in a combat stance.
				 * Non-Players will only appear this way when close to their target
				 **/
