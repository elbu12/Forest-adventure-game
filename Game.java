...
public class Game implements ActionListener, KeyListener{
    ...
    Later, mapmaking methods will fill them in with the appropriate content.
		...
		//Ask user if they want to play the game or the tutorial
		...
		JTextArea initialTextArea = new JTextArea("Welcome to the game! Would you like to\nstart a new game or play the tutorial?");
		final DialogButton startGameButton = new DialogButton("Start new game") {
			public void press() {
				initialDialog.setVisible(false); //Sometimes this does not work...?
				makeRegularGame();

				initialDialog.setVisible(false); //Sometimes this does not work...?
			}
		};
		DialogButton playTutorialButton = new DialogButton("Tutorial") {
			public void press() {
				initialDialog.remove(this);
				initialDialog.remove(startGameButton);
				playTutorial(initialDialog, initialTextArea, gbc);
			}
		};
		initialDialog.add(initialTextArea, gbc);
		gbc.gridy++;
		initialDialog.add(startGameButton, gbc);
		gbc.gridx++;
		initialDialog.add(playTutorialButton);
		gbc.gridy = 0;
		gbc.gridx = 0;
		initialTextArea.setEditable(false);
		initialDialog.pack();
		initialDialog.setLocation(frame.getX() + (frame.getWidth()-initialDialog.getWidth())/2, 
				frame.getY() + (frame.getHeight()-initialDialog.getHeight())/2);
		initialDialog.setVisible(true);
	}

	public void playTutorial(JDialog initialDialog, JTextArea initialTextArea, GridBagConstraints gbc) {
		...
		initialDialog.setLocation(initialDialog.getX(), frame.getY()+(frame.getHeight()*3/4));
		player.moveTo(worldWidth/2, worldHeight/2, getIntersectingTiles(player, worldWidth/2, worldHeight/2));
		initialTextArea.setText("You are the person in the center.\n(It's an overhead view)\nMove with the arrow keys.");
		DialogButton initialButton = new DialogButton("Next") {
			int tutorialState = 0;
			Monster tutorialMonster = new Monster(player) {
				public void interact() {
					getResponse("The monster does not react.");
				}
				public void die() {
					if (tutorialState > 4) {
						super.die();
						...
						moveTo(player.x, tempy, getIntersectingTiles(this, player.x, tempy));
					}
				}
			...
			Fencer tutorialFencer = new Fencer(player) {
				public void interact() {
					getResponse("'Hello.'");
				}
				public void die() {
				...
				public void reset() {
					...
					if (tutorialState == 10) {
						...
						fightingStyle = (r == 0 ? SolidActor.IMMEDIATE_ATTACK : r == 1 ? SolidActor.COUNTERATTACK : SolidActor.FEINT_THEN_ATTACK);
					}
				}
				public int getDamage() {
					//When this hits, just relocate it
			...
			public void press() {
				...
				case 1:
					initialTextArea.setText("Use 'd' and 'g' keys to turn. Use 'p' to pause.");
					break;
				case 2:
					initialTextArea.setText("When not in combat mode, press space to interact with\nobjects and people. Try interacting with this monster.");
		...
		Fencer stalkerTalker = new Fencer(player, x + (w/2), y + (h/2)){
			public void act(){
				...
				case STAND_UNTIL_DETECT_THEN_ATTACK:
					...
					else if (distanceBetween2(this.x, this.y, target.x, target.y) < 3.5*3.5){
						//Close enough to talk
						getResponse(stalkers.size() > 0 ?
				...
				case STAND:
					...
					else if (distanceBetween2(this.x, this.y, target.x, target.y) < 1.5*1.5){
			...
			public void interact() {
				getResponse("'Sorry! Please don't kill me!'");
			}
		};
