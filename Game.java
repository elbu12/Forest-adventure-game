...
CODE QUESTION:
  Pathfinding was not working. Stalkers would attempt to navigate toward their
  starting locations if out of range. Then I changed it, such that they now
  only try to navigate toward that point if they are not already there. That
  fixed it. Why?

Should the northeast building in the abandoned city be a tower,
with a second level?
...
why is initializing a textArea so slow?
Do I need to hide the initialization in a different thread?

Add rubble, tables, benches, other spider objects

Thoughts on pathfinding:

 Perhaps, when pathfinding toward a target solid, rather than removing the solid
 and pathfinding toward its center, keep the solid and track when an intersection
 occurs with it. Note: tried this and it led to freezing?
 
 How well should enemies coordinate to avoid one blocking another?
 
 Should pathFindingIterations decrease as the number of navigators navigating increases?
...
public class Game implements ActionListener, KeyListener{
  ...
  static ArrayList <Actor> actors = new ArrayList <Actor> (10);
  ...
  //Whether you killed the monster
  public static boolean monsterDead = false;
  
  //Whether you know about the widow's heart
  public static boolean knowAboutGoal = false;
  
  //Used for dancers in the village
  public static VillageDance dance = new VillageDance();
  ...
  static JPanel panel = new JPanel(){
    public void paintComponent(Graphics g){
      ...
      g.fillRect(frameWidth, healthBarHeight*(player.maxHealth-player.health)/player.maxHealth, healthBarWidth,
  ...
  public Game(){
    ...
    //These EditableIntArrays represent nodes (used for pathfinding) within individual tiles
    ...
    actors.add(Tile.riverAnimator);
    
    /**
    CREATING THE MAP:
    All tiles are initially made as empty "undesignated" tiles.
    ...
    **/
    ...
    /**
    Make a 10-tile thick perimeter around the map.
    5 blank tiles, then 5 of forest floor, all full of trees.
    **/
    ...
    **/
    ...
    makeDealerSister(20, 117);
    makeJunkieCamp(79, 30);
    ...
    //Create the welcome message in the permanent frame
    makePermanentMessage("Welcome to the game!", "Press the arrow keys to move and d/g to turn.\n" +
                         "Press space to interact with an object before you.\n" +
                         "Press p to pause.\n\n"+
  ...
  //Button for pausing
  final static DialogButton resumeButton = new DialogButton("Resume"){public void press(){}};
  ...
  /**
  Methods for getting responses with a dialog window.
  getResponse(s) will pause the game to ask for a response.
  getResponse will restart the game timer afterward.
  getResponses will not. Use it for multiple consecutive messages.
  
  To make a pop-up dialog window, call getResponse(s) with a String (message) and
  some amount of DialogButtons. The String is the text to display. The buttons
  are the buttons to be available as options. If no button is given, a default
  "Ok" button will be used.
  ...
  **/
  ...
  Remember, when relevant, to check for collisions with a Solid's intended destination,
  ...
  **/
  ...
  public static boolean circleIntersectRectangle(double x1, double y1, double r, double x2, double y2,
                                                 double w, double h){
    if (y1 < y2 - (h*0.5)){
  ...
  public static boolean intersect(double x1, double y1, double w1, double h1, double s1, double x2, double y2,
                                  double w2, double h2, double s2){
  ...
  public static boolean intersect (Solid s1, double x0, double y0, double dx, double dy, Solid s2){
    if (s1.shape == Game.CIRCLE){
      if (s2.shape == Game.CIRCLE){
        ...
        /**
        basic idea: treat the problem as if the circle is a point (its center)
        and the rectangle is expanded, with rounded corners. Does this moving
        point (ie line segment) intersect this rounded rectangle?
        
        The rounded rectangle is essentially two overlapping rectangles, forming
        a "+" sign, with circles in the corners
        ...
        **/
        if (dx > 0){
          if (dy > 0){
            return circleIntersectLineSegment(s2.x-(0.5*s2.width),s2.y-(0.5*s2.height),0.5*s1.width,x0,y0,dx,dy)
              || rectangleIntersectLineSegment(s2.x,s2.y,s1.width+s2.width,s2.height,x0,y0,dx,dy)
              || rectangleIntersectLineSegment(s2.x,s2.y,s2.width,s1.height+s2.height,x0,y0,dx,dy)
              || circleIntersectLineSegment(s2.x-(0.5*s2.width),s2.y+(0.5*s2.height),0.5*s1.width,x0,y0,dx,dy)
              || circleIntersectLineSegment(s2.x+(0.5*s2.width),s2.y-(0.5*s2.height),0.5*s1.width,x0,y0,dx,dy);
          }
          else {
            return circleIntersectLineSegment(s2.x-(0.5*s2.width),s2.y+(0.5*s2.height),0.5*s1.width,x0,y0,dx,dy)
      ...
      if (s2.shape == Game.CIRCLE){
        /**
        Still use the circle/rectangle intersection test mentioned above, but adjust it
        The test above assumes the circle is moving. So swap the order of these, such that
        the circle moves. It moves in the opposite direction.
        Everything gets shifted over to put s1 at its own location, rather than (x1,x2)
        **/
        return intersect(s2, s2.x+s1.x-x0, s2.y+s1.y-y0, s2.x+s1.x-x0-dx, s2.y+s1.y-y0-dy, s1);
      }
      else if (s2.shape == Game.RECTANGLE){
        //note that order of s1 and s2 are reversed because s1 is the moving solid,
        //but the method requires the line segment (the moving part) come second
        return rectangleIntersectLineSegment(s2.x, s2.y, 0.5*(s1.width+s2.width), 0.5*(s1.height+s2.height), x0, y0,
                                             dx, dy);
      }
      else if (s2.shape == Game.POINT){
        //note that order of s1 and s2 are reversed because s1 is the moving solid,
  ...
  //tests if circle with center (cx,cy) and radius r intersects line segment spanning
  //from (lx,ly) to (lx+dx,ly+dy)
  public static boolean circleIntersectLineSegment(double cx, double cy, double r, double lx, double ly,
                                                   double dx, double dy){
    /**
    distance squared, from line segment to circle center, is:
      ( cx - (lx + kdx))^2 + ( cy - (ly + kdy))^2
    derivative is:
      2(cx - (lx + kdx))dx + 2(cy - (ly + kdy))dy
    when derivative is zero, line is closest to circle. This happens when:
      k = ( dx(cx-lx) + dy(cy-ly) )/(dx^2 + dy^2)
    **/
  ...
  public static boolean rectangleIntersectLineSegment(double rx, double ry, double rw, double rh, double lx, double ly,
                                                      double dx, double dy){
  ...
  /**
  This is the beginning of the "where" methods. They determine where an intersection
  occurs as an object moves along a given path.
  
  They do not determine the distance this object could travel before intersecting.
  Rather, they find a coefficient, such that, the traveler's position, plus the
  vector of its movement, times this coefficient, is the point where the intersection
  occurs.
  
  Eg, if object o moves from point p to point (p+v), and a "where" method determines
  an intersection occurs at i, then the actual point of the intersection is 
  p + iv
  
  **/
  /**
  Determines where a point moving from (x0,y0) to (x0+dx,y0+dy) intersects a circle at
  (cx,cy) with radius r.
  **/
  public static double wherePointIntersectCircle(double x0, double y0, double dx, double dy,
                                                            double cx, double cy, double r){
    /**
    Line is (x0,y0)+t(dx,dy)
    Circle is (x-cx)^2 + (y-cy)^2 = r^2
    (x0+tdx-cx)^2 + (y0+tdy-cy)^2 = r^2
    Let a := x0-cx
    Let b := y0-cy
    (tdx+a)^2 + (tdy+b)^2 = r^2
    (tdx)^2 + 2atdx + a^2 + (tdy)^2 + 2btdy + b^2 = r^2
    (dx^2 + dy^2)t^2 + 2(adx + bdy)t + (a^2 + b^2 - r^2) = 0
    Let i := (dx^2 + dy^2)
    Let j := 2(adx + bdy)
    Let k := (a^2 + b^2 - r^2)
    it^2 + jt + k = 0
  ...
  //Determines where a circle with radius r1, moving from (x0,y0) to (x0+dx,y0+dy)
  //intersects a circle at (cx,cy) with radius r2
  public static double whereCircleIntersectCircle(double r1, double x0, double y0, double dx, double dy,
                                                             double cx, double cy, double r2){
    return wherePointIntersectCircle(x0, y0, dx, dy, cx, cy, r1+r2);
  }
  
  //Determines where a point, moving from (x0,y0) to (x0+dx,y0+dy), intersects a point
  //at (px,py). Assume points have radius pointRadius
  public static double wherePointIntersectPoint(double x0, double y0, double dx, double dy,
                                                           double px, double py){
    return wherePointIntersectCircle(x0, y0, dx, dy, px, py, pointRadius);
  }
 
  //Determines where a point, moving from (x0,y0) to (x0+dx,y0+dy), intersects a rectangle
  //with center (rx,ry) and (width,height) of (w,h)
  public static double wherePointIntersectRectangle(double x0, double y0, double dx, double dy,
                                                               double rx, double ry, double w, double h){
    double t0 = Double.POSITIVE_INFINITY;
    if (dx != 0){
      /**
      Intersection occurs when x0+tdx = rx +/- (w*0.5)
      t = (rx - x0 +/- (w*0.5))/dx
      **/
    ...
    if (dy != 0){
      /**
      Intersection occurs when y0+tdy = ry +/- (h*0.5)
      t = (ry - y0 +/- (h*0.5))/dy
      **/
  ...
  //Determines where a rectangle, with (width,height) of (w1,h1), moving from (x0,y0) to
  //(x0+dx,y0+dy), intersects a rectangle with center (rx,ry) and (width,height) (w2,h2)
  public static double whereRectangleIntersectRectangle(double w1, double h1, double x0, double y0,
                                                                   double dx, double dy, double rx, double ry,
                                                                   double w2, double h2){
    return wherePointIntersectRectangle(x0, y0, dx, dy, rx, ry, w1+w2, h1+h2);
  }
  
  //Determines where a circle, with radius r, moving from (x0,y0) to (x0+dx,y0+dy), intersects
  //a rectangle with center (rx,ry) and (width,height) of (w,h)
  public static double whereCircleIntersectRectangle(double r, double x0, double y0, double dx, double dy,
                                                                double rx, double ry, double w, double h){
  ...
  public static double whereRectangleIntersectCircle(double w, double h, double x0, double y0,
                                                                double dx, double dy, double cx, double cy, double r){
    return whereCircleIntersectRectangle(r, cx, cy, -dx, -dy, x0, y0, w, h);
  }
  
  //This determines where a point, moving from (x0,y0) to (x0+dx,y0+dy), intersects solid s
  ...
  //This determines where solid s1, moving from (x0,y0) to (x0+dx,y0+dy), intersects solid s2
  public static double whereSolidIntersectSolid(Solid s1, double x0, double y0, double dx, double dy, Solid s2){
    if (s1.shape == POINT){
      ...
      if (s2.shape == POINT){
        ...
        return whereRectangleIntersectRectangle(s1.width, s1.height, x0, y0, dx, dy, s2.x, s2.y, s2.width, s2.height);
      }
      else {
        return Double.POSITIVE_INFINITY;
      }
  ...
  //this version is used for the getIntersectingTiles version for lines (rather than moving solids)
  ...
  //getIntersectingTiles will return intersectingTiles, which now contains the tiles
  //intersecting a given solid. If no x/y coordinates are given, it will
  //look at the object's current location. If x/y coordinates are given, it will look
  ...
  public static ArrayList <Tile> getIntersectingTiles(Solid s, double x, double y){
    ...
    for (int i=xstart; i<xend; i++){
      for (int j=ystart; j<yend; j++){
        if (intersect(x, y, s.width, s.height, s.shape, 0.5+(int) (i), 0.5+(int) (j), 1, 1, Game.RECTANGLE)){
          intersectingTiles.add(tiles[i][j]);
        }
      }
    }
  ...
  /**
  getIntersecting Tiles, with two x/y coordinate pairs, and a solid
  will find tiles intersecting the space through which s would travel,
  if moving from (x0,y0) to (x1,y1).
  Use it to determine if straight-line movement is possible.
  ...
  **/
  ...
  public static ArrayList <Tile> getIntersectingTiles(Solid s, double x0, double y0, double x1, double y1){
    ...
    //get the left and right edges before and after the translation
    int left0 = (int)(x0-(s.width*0.5));
    int left1 = (int)(x1-(s.width*0.5));
    int right0 = (int)(left0 + s.width);
    int right1 = (int)(left1 + s.width);
    if ((int)(left0) == (int)(left1) && (int)(right0) == (int)(right1)){
      //If the x difference is too small, just treat it as vertical movement
      int xmin = left0;
      xmin = Math.max(xmin, 0);
      double xmax = x0 + (s.width*0.5);
      xmax = Math.min(worldWidth, xmax);
      int ymin = (int)(Math.min(y0, y1) - (s.height*0.5));
      double ymax = Math.max(y0, y1) + (s.height*0.5);
      for (int i=xmin; i<xmax;i++){
        addIntersectingTiles(i, ymin, ymax);
      }
    }
    else if (y1 == y0){
      //If the y difference is too small, just treat it as horizontal movement
      int xmin = (Math.min(left0, left1));
      xmin = Math.max(xmin, 0);
      double xmax = Math.max(x0, x1) + (s.width*0.5);
      xmax = Math.min(worldWidth, xmax);
      int ymin = (int)(y0 - (s.height*0.5));
      double ymax = y0 + (s.height*0.5);
      for (int i=xmin; i<xmax;i++){
        addIntersectingTiles(i, ymin, ymax);
      }
    }
      ...
      //diagonal movement
      //Model the path s takes as a parallelogram
      //Go column-by-column
      //Note that this will overestimate the tiles s intersects on its path
      if (x1 < x0){
        //Movment is left-to-right. Reverse direction if necessary
        x1 = x0 + x1;
        x0 = x1 - x0;
        x1 = x1 - x0;
        y1 = y0 + y1;
      ...
      //parallelogram has two diagonal edges; call them e1 and e2
      //they have starting points, e1x, e1y, etc
      //these lines pass through opposite corners of the rectangle containing s
      double e1x;
      double e1y;
      double e2x;
      double e2y;
      ...
      //this parallelogram has vertical edges; move e1 and e2 to tile edges
      int xmin = (int) (Math.min(e1x, e2x));
      xmin = Math.max(xmin, 0);
      e1y += slope * (xmin - e1x);
      e1x = xmin;
      e2y += slope * (xmin - e2x);
      e2x = xmin;
      //determine which of these corners has a lesser y-value
      double ymin = Math.min(e1y, e2y);
      //determine the vertical difference between them
      double ydif = Math.max(e1y, e2y) - ymin;
      //how far does this go in the x direction?
      double xmax = x1 + (s.width*0.5);
      xmax = Math.min(xmax, worldWidth);
      //track the y-value of the diagonal edge with a lesser y-value
      //do this with ymin, which will change as x increases
      for (int i=xmin; i<xmax; i++){
        double ymax = Math.max(ymin+ydif, ymin+ydif+slope);
        //y starts at the least point. Not less than zero!
        addIntersectingTiles(i, (int)(Math.min(ymin, ymin+slope)), ymax);
        ymin += slope;
      }
    }
    return intersectingTiles;
 ...
 **/
  //This is the new version! It does not increment by a "slope"
  //Delete the methods above, once the ones below are done
  public static ArrayList <Tile> getIntersectingTiles(Solid s, double x0, double y0, double x1, double y1){
    if (s.shape == Game.POINT){
      //s is a point; use method for infinitesimally thin line segments
      return getIntersectingTiles(x0, y0, x1, y1);
    ...
    /**
    If we made it this far, it means the movement is diagonal.
    The object (assumed to be rectangular) moves along a "thick line," but bounded
    with left, right, top, and bottom edges. The actual space spanned by a moving
  ...
  /**
  getIntersecting Tiles, with two x/y coordinate pairs, but no solid,
  will find tiles intersecting a line segment.
  **/
  ...
  //returns the first solid encountered while moving from (x0,y0) to (x0+dx,y0+dy);
  ...
  public static Solid getIntersectingSolid(Solid s, double x0, double y0, double dx, double dy){
    ...
    //t0 is the coefficient of the parameterization of the path at which an intersection occurs
  ...
  /**
  This returns the first obstacle s would encounter moving from (x0,y0)
  to (x0+dx,y0+dy). "Obstacles" include solids and unwalkable tiles if
  s "walks."
  
  This was used for an older version of pathfinding.
  **/
  public static Obstacle getIntersectingObstacle(Solid s, double x0, double y0, double dx, double dy, boolean reuseIntersectingTiles){
    ...
    for (Tile tile : intersectingTiles){
      ...
      if (s.walks && !tile.walkable && rectangleIntersectLineSegment(tile.x+0.5, tile.y+0.5, 1, 1, x0, y0, dx, dy)){
  ...
  **/
  ...
  //The setNodes method gets the nodes within a tile and adds them to the appropriate tileNodes
  //The input parameter "s" is just there to prevent creating nodes around s itself
  public static void setNodes(int i, int j, Solid s, double destinationx, double destinationy){
    ...
    relevantSolids.clear();
    /**
     * This method should find all relevant nodes within this tile and place them within
     * this tile's tileNodes collection.
     * 
     * A solid in an adjacent tile may create a node in this tile, as the node is not
     * actually at the solid's corner. Rather, it must leave space for s's width.
     * 
     * Thus, this actually should look at tile (i,j) and the 8 surrounding tiles, but
     * keep only nodes actually in tile (i,j).
    **/
  ...
  /**
  getNextStep finds the next step in the path s could take from (x0,y0) to (x1,y1).
  It will generally try to find the shortest path, but only to an extent.
  
  It implements the A-Star algorithm. The nodes are tile centers and corners of
  obstructions.
  
  After finding a path, the path should be optimized to avoid unnecessary zig-zags.
  **/
...
//THIS IS THE AREA TO REWRITE
      ...
      for (int i=tilex-1; i<tilex+2; i++){
        ...
        for (int j=tiley-1; j<tiley+2; j++){
          ...
          for (int k=0; k<nodes.size(); k++){
            ...
            if (canMove(s, currentx, currenty,
                        nodex.get(nodek)-currentx, nodey.get(nodek)-currenty)){
              neighbors.add(nodek);
            }
          }
        }
      ...
      for (int i=0; i<neighbors.size(); i++){
        ...
        double newDistanceTo = distanceTo.get(current) + distanceBetween(currentx, currenty,
                                                                     nodex.get(neighbor), nodey.get(neighbor));
    ...
    //If we made it out of the loop, then a path to the destination could not be found.
  ...
  **/
  ...
  /**
  optimizePath takes a path, represented as the node at the end, and
  connected by parent attributes, and finds the furthest node along this path
  that can be reached directly from the starting node.
  **/
  public static double[] optimizePath(Solid s, int node){
    ...
    while (!canMove(s, startx, starty, nodex.get(node)-startx, nodey.get(node)-starty)){
      //If s cannot move from start to "node," replace node with its parent, one step back
  ...
  /**
  The version of canMove with two x/y pairs determines if s can move from (x0,y0)
  to (x0+dx,y0+dy). It will check for collisions along the way, not just at the final
  destination
  **/
  ...
  /**
  getClosestSolidInCollection will find the Solid in Collection c,
  closest to s, within a range of r. If no such Solid exists, it will return s.
  ...
  **/
  ...
  /**
  This is used by getClosestSolidInCollection to add Solids to relevantSolids if
  ...
  **/
  ...
  /**
  This method will take a given Navigator s, find the direction that takes it away
  from its enemies, and write that direction to s.tx and s.ty
  ...
  **/
  ...
  This one makes an impenetrable wall of trees from (x0,y0) to (x0+dx,y0+dy),
  with width w. It makes trees iteratively, with each placed somewhere on a
  "circle" around the previous tree, with the width and circle radius set such
  that the space between consecutive trees is too small for the player to pass
  through it.
  **/
  public static void makeTreeWall(double x0, double y0, double dx, double dy, double w){
    double length = Math.sqrt((dx*dx) + (dy*dy));
    //get unit vectors parallel (u) and perpendicular (p) to wall
    double ux = dx/length;
    double uy = dy/length;
    double px = uy;
    double py = -ux;
    //maxWidth is the maximum width of a tree here, as it can't be wider than the wall
    double maxWidth = Math.min(w, Tree.maxWidth);
    Tree lastTree = new Tree(Tree.minWidth + (Math.random()*maxWidth));
    double newu = lastTree.width*0.5;
    double newp = (0.5*(lastTree.width-w)) + (Math.random()*(w-lastTree.width));
    double newx = x0 + (newu*ux) + (newp*px);
    double newy = y0 + (newu*uy) + (newp*py);
    lastTree.moveTo(newx, newy, getIntersectingTiles(lastTree, newx, newy));
    double oldu = (lastTree.x*ux) + (lastTree.y*uy);
    double oldp = (lastTree.x*px) + (lastTree.y*py);
    //determine p value of wall's center line
    double midp = (x0*px) + (y0*py);
    //while we have not yet reached the tree that might end the wall:
    while (distanceBetween(lastTree.x,lastTree.y, x0+dx-(maxWidth*0.5*ux), y0+dy-(maxWidth*0.5*uy)) >
           (0.5*(lastTree.width+maxWidth))+treeGap){
      //make a new tree
      Tree newTree = new Tree(Tree.minWidth + (Math.random()*maxWidth));
      //determine the gap between this and the previous one. Keep it <= Player.width*0.25
      double gap = Math.random()*treeGap;
      /**
      newTree's p value must be between the edges of the tree wall, and also
      no more than gap+(lastTree.width*0.5) from lastTree
      **/
      double minp = Math.max(midp + ((newTree.width-w)*0.5), oldp - (gap + (0.5*(lastTree.width+newTree.width))));
      double maxp = Math.min(midp - ((newTree.width-w)*0.5), oldp + (gap + (0.5*(lastTree.width+newTree.width))));
      //Now pick a new p
      newp = minp + (Math.random()*(maxp-minp));
      /**
      Now find an equation for newu as a function for newp and use it to determine newu
      (newu-oldu)^2 + (newp-oldp)^2 = ( gap + 0.5*(width+width) )^2
      (newu-oldu)^2 = ( gap + 0.5*(width+width) )^2 - (newp-oldp)^2
      newu-oldu = √( ( gap + 0.5*(width+width) )^2 - (newp-oldp)^2 )
      newu = oldu + √( ( gap + 0.5*(width+width) )^2 - (newp-oldp)^2 )
      When dealing with the sign of the radical, always choose the greater value, because
      we want to move in the u-direction
      **/
      double gapPlusHalfWidthSum = gap + (0.5*(lastTree.width + newTree.width));
      double pDifference = newp-oldp;
      newu = oldu + Math.sqrt( (gapPlusHalfWidthSum*gapPlusHalfWidthSum) - (pDifference*pDifference) );
      //We now have newp and newu. Time to place newTree!
      newx = (newu*ux) + (newp*px);
      newy = (newu*uy) + (newp*py);
      newTree.moveTo(newx, newy, getIntersectingTiles(newTree, newx, newy));
      lastTree = newTree;
      oldu = newu;
      oldp = newp;
    }
    /**
    Now to place the final tree, with location (newu, newp) and width newTree.width.
    The idea is to place it at the edge of the tree wall, with appropriate width.
    The limiting propositions become:
      newp + (newTree.width*0.5) <= midp + (w*0.5)
      newp - (newTree.width*0.5) >= midp - (w*0.5)
      newu + (width*0.5) = ux*(x0+dx) + uy*(y0+dy)
      newTree.width >= Tree.minWidth
      newTree.width <= maxWidth
      (newu-oldu)^2 + (newp-oldp)^2 <= (((newTree.width+lastTree.width) * 0.5) + gap)^2
      (newu-oldu)^2 + (newp-oldp)^2 >= ((newTree.width+lastTree.width) * 0.5)^2
      
    For a given newp,
      ww <= [ (w*0.5) + midp - newp ]*2
      ww <= [ (w*0.5) + newp - midp ]*2
      ww <= maxWidth
      
      ww >= Tree.minWidth
      
      This newp value is impossible if [ (w*0.5) - |midp - newp| ]*2 < Tree.minWidth
      Range of acceptable p values is therefore:
      
      [ (w*0.5) - (midp - newp) ]*2 = Tree.minWidth
      0.5*w - midp + newp = 0.5*Tree.minWidth
      newp = 0.5*Tree.minWidth + midp - 0.5*w for minimum p
      
      [ (w*0.5) + (midp - newp) ]*2 = Tree.minWidth
      0.5*w + midp - newp = 0.5*Tree.minWidth
      newp = 0.5*w + midp - 0.5*Tree.minWidth for maximum p
      
    Therefore:
      0.5*Tree.minWidth + midp - 0.5*w <= p <= 0.5*w + midp - 0.5*Tree.minWidth
      
    Moving on to last propositions:
      
      newu + (width*0.5) = ux*(x0+dx) + uy*(y0+dy)
      newu = ux*(x0+dx) + uy*(y0+dy) - (ww*0.5)
      
    Let A = ux*(x0+dx) + uy*(y0+dy)
    A is basically the u-value of the edge of the tree wall
    ...
    Last two boundaries:
      ...
      (A - (ww*0.5) -oldu)^2 + (newp-oldp)^2 = (((newTree.width+lastTree.width) * 0.5) + gap)^2
      (A - (ww*0.5) -oldu)^2 + (newp-oldp)^2 = ((newTree.width+lastTree.width) * 0.5)^2
      
    Let B = A - oldu
    B is basically the distance from lastTree to edge of tree wall
      
      (B - (ww*0.5))^2 + (newp-oldp)^2 = (((newTree.width+lastTree.width) * 0.5) + gap)^2
      (B - (ww*0.5))^2 + (newp-oldp)^2 = ((newTree.width+lastTree.width) * 0.5)^2
      
      (B - (ww*0.5))^2 + (newp-oldp)^2 = (ww*0.5 + lastTree.width*0.5 + gap)^2
      (B - (ww*0.5))^2 + (newp-oldp)^2 = (ww*0.5 + lastTree.width*0.5)^2
      
    Let C = lastTree.width*0.5
      
      (B - (ww*0.5))^2 + (newp-oldp)^2 = (ww*0.5 + C + treeGap)^2
      (B - (ww*0.5))^2 + (newp-oldp)^2 = (ww*0.5 + C)^2
      
    Let D = C + treeGap
    D is basically maximum distance from lastTree center to newTree edge
      
      (B - (ww*0.5))^2 + (newp-oldp)^2 = (ww*0.5 + D)^2
      (B - (ww*0.5))^2 + (newp-oldp)^2 = (ww*0.5 + C)^2
      
    Let E = (newp-oldp)^2
      
      (B - (ww*0.5))^2 + E = (ww*0.5 + D)^2
      (B - (ww*0.5))^2 + E = (ww*0.5 + C)^2
      
      0.25*ww^2 - B*ww + B^2 + E = 0.25*ww^2 + D*ww + D^2  (setting a minimum for ww)
      0.25*ww^2 - B*ww + B^2 + E = 0.25*ww^2 + C*ww + C^2  (setting a maximum for ww)
      
      (B+D)*ww = E + B*B - D*D
      (B+C)*ww = E + B*B - C*C
      
      ww = [ E + B*B - D*D ] / (B+D)  (minimum ww value)
      ww = [ E + B*B - C*C ] / (B+C)  (maximum ww value)
    ...
    And ww maxima are:
      ...
      maxWidth
      **/
    double minp = (0.5*Tree.minWidth) + midp - (0.5*w);
    double maxp = (0.5*w) + midp - (0.5*Tree.minWidth);
    newp = minp + (Math.random()*(maxp-minp));
    
    double A = (ux*(x0+dx)) + (uy*(y0+dy));
    double B = A - oldu;
    double C = lastTree.width*0.5;
    double D = C + treeGap;
    double E = (newp-oldp)*(newp-oldp);
    
    double minWid = Math.max(Tree.minWidth, ( E + (B*B) - (D*D) ) / (B+D));
    double maxWid = Math.min(maxWidth, ( E + (B*B) - (C*C) ) / (B+C));
    
    Tree newTree = new Tree(minWid + (Math.random()*(maxWid-minWid)));
    
    newu = A - (newTree.width*0.5);
    newx = (ux*newu) + (px*newp);
    newy = (uy*newu) + (py*newp);
    
    //Now check for possible overlap with lastTree
    if (circleIntersectCircle(lastTree.x, lastTree.y, lastTree.width*0.5, newx, newy, newTree.width*0.5)){
      //Abort! Else, a new tree will be placed within the last one!
      return;
    }
    newTree.moveTo(newx, newy, getIntersectingTiles(lastTree, newx, newy));
  }
  
  /**
  fillWithTrees will attempt to fill the designated rectangle with trees.
  It uses the list remainingTiles to track which tiles have already
  been addressed.
  **/
  ...
  public static void fillWithTrees(int x, int y, int width, int height, double gap){
    /**
    This will iterate through the tiles in the designated region, but in random order.
    For each tile, it will attempt to place a tree of random width, in a random place
    in the chosen tile. It will then remove this tile from the list. If a "gap" is
    chosen, it will try to leave that much
    space blank beside each tree, to leave a gap through which navigators can move.
    ...
    **/
    ...
    while (!remainingTiles.isEmpty()){
      ...
      /**
      Pick a random width. Can a tree be placed here?
      Note that the width is artificially large, to leave a gap around the tree.
      Later, the tree's correct width will be set.
      **/
      ...
      for (int i=0; i<2; i++){
        ...
        else if (ex - (e.width*0.5) < x || ex + (e.width*0.5) > x + width ||
            ey - (e.width*0.5) < y || ey + (e.width*0.5) > y + height){
  ...
  /**
  makeFencers places n fencers within the rectangle with corner (x,y) and width/height (w,h)
  These fencers will have randomized fightingStyles. Also, when one notices you, the others
  will, too.
  **/
  ...
  **/
  public static void makeWallWithDoor(double x, double y, double width, double height, boolean vertical, double doorWidth){
    if (vertical){
  ...
  /**
  makeBuilding will make a rectangular building, with northwest corner at (x,y) and internal width and 
  height as given. "Thickness" refers to the thickness of the walls.
  
  This building also has doorways. Each doorway is determined by its corresponding boolean. Doorways are
  placed in the center of their walls. They are doorWidth in width.
  
  The building's interior is stone floor.
  
  Note that the building's entire width is width + (2*thickness)! Width is just the width of the open
  inside area!
  **/
  ...
  public static void makeOldManArea(int x, int y, int w, int h){
    ...
    Talker monsterMan = new Talker(hx, hy+4, Math.PI, new Color(90, 90, 30), player){
      ...
      public ScaledImage draw(){
        if (!haveBeenAlerted && distanceBetween2(x, y, target.x, target.y) < screenWidth*screenWidth*0.2){
          haveBeenAlerted = true;
          getResponse("You see an old man standing outside a cabin.\nHe seems friendly.");
        }
        return super.draw();
      }
      public void interact(){
        if (monsterDead){
          getResponse("'I see you killed the monster. Thank you!'");
        }
        else {
          getResponse("'Beware of the monster that lurks west of here.\nDon't go there unarmed!'");
        }
      }
      public void hit(int damage){
        getResponse("To talk with the old man, press space when not in combat mode.");
      }
    };
    fillWithTrees(x, y, w, h, 2);
  }
  ...
  public void makeMonsterArea(int x, int y, int w, int h){
    ...
    //The monster
    new WanderingMonster(x+(w/2), y+(h/2), x, y+4, w, h-4, player){
      protected void die(){
        monsterDead = true;
        super.die();
      }
  ...
  /**
  makes an area with the old woman at the top, presumably by the stream
  above her. She tells you about the game's general goal.
  Remember to put this next to water!
  **/
  public static void makeOldWomanArea(int x, int y, int w, int h){
    ...
    Talker oldWoman = new Talker(x+(h*0.5)-0.5+Math.random(), y, 0, new Color(90, 90, 30), player){
      boolean haveBeenAlerted = false;
      public ScaledImage draw(){
        if (!haveBeenAlerted && distanceBetween2(x, y, target.x, target.y) < screenWidth*screenWidth*0.2){
          haveBeenAlerted = true;
          getResponse("You see an old woman washing something in the river.\nShe seems friendly.");
        }
        return super.draw();
      }
      public void interact(){
          getResponse("The old woman tells you about the legend of the Widow's Heart,\n" +
      ...
      public void hit(int damage){
        getResponse("To talk with the old woman, press space when not in combat mode.");
      }
    };
  ...
  **/
  ...
  (x,y) is the northwest corner of the area
  (w,h) is the width and height of the area
  vertical is whether this is a column
    if vertical, then every building will have width of w, but with variable height
  northeast is whether the buildings have doors on the north or east side
    vertical building columns have doors on their west or east sides
  n is the number of buildings
  m is the minimum building width/height (depending on whether this is a row/column)
  a is "alley width," the space between buildings
  
  Note that this method does not set tile types, except for the stone floors inside buildings
  ...
  **/
  ...
  public void makeWalledVillage(int x, int y){
    ...
    /**
    //make their beds - this has been turned off!
    new Bed(x+4, y+20, 4, 2, Math.PI*0.5);
    new Bed(x+4, y+32, 4, 2, Math.PI*0.5);
    new Bed(x+4, y+46, 4, 2, Math.PI*0.5);
    **/
    ...
    //corpse within tower
    Corpse lenny = new Corpse(1.3*0.63, 1.3*0.96){
    ...
    WanderingMonster wamo = new WanderingMonster(x+5, y+3, x+2, y+3, 30, 13, player){
    ...
    //now make all tiles unable to have trees, to avoid unwanted randomized future placements
    for (int i=x; i<x+width; i++){
      for (int j=y; j<y+height; j++){
  ...
  /**
  The junkie/sister situation is as follows:
  A junkie accidentally killed his dealer while trying to get more drugs.
  The dealer's sister is aware of the event, but is unable to locate the junkie,
  who fled after the incident. If you meet the dealer's sister, she will describe
  the junkie to you.
  
  The junkie is hiding at a certain camp, which contains a cold fire pit.
  If you reach the camp before having met the sister, you will see the remains of
  a camp, but not the junkie himself. Once you meet the sister, if you visit the
  camp, the junkie will be there. You can confront him. If you kill him, you can
  return to the sister for a reward.
  
  Alternatively, if you decline to kill him, he will flee. You can then return to
  the sister's house and inform her of what happened. She will thank you and go
  to the camp to investigate. This leaves her home unoccupied. You can exploit
  the opportunity and search it for the reward you would have received.
  
  The dealer's sister is a Navigator. When her intention is STAND, she will talk.
  If you tell her about the junkie's camp, her intention changes to MOVE and
  she will walk to the camp.
  **/
  ...
  public void makeDealerSister(int x, int y){
    int w = 30;
    int h = 30;
    for (int i=x; i<x+w; i++){
      for (int j=y; j<y+h; j++){
    ...
    /**
    The table with the shard should possibly be
    replaced with something else, like a dresser or a (treasure) chest,
    when I determine how to draw it recgonizably
    **/
    ...
    dealerSister = new Navigator(1, 1){
      BufferedImage image = Fencer.getBallAndStick(new Color(90, 90, 30))[0];
      public void interact(){
        if (junkieState == UNAWARE || junkieState == AWARE || junkieState == CHOOSING){
          dealerSisterTalk(0);
        }
        else if (junkieState == KILLED){
          if (receivedRewardForJunkie){
            dealerSisterTalk(2);
          }
          else {
            dealerSisterTalk(1);
      ...
      public void hit(int damage){
        getResponse("To talk with the woman, press space when not in combat mode.");
      }
      public void act(){
        if (intention == MOVE){
          if (dieWhenFar(15)){
            //remove if out of sight
            dealerSisterGone = true;
    ...
    dealerSister.behavior = Navigator.STAND;
    dealerSister.intention = SolidActor.STAND;
    dealerSister.direction = Math.PI;
    dealerSister.place(hx, hy+0.15);
    dealerSister.slowSpeed = player.slowSpeed;
    dealerSister.speed = dealerSister.slowSpeed;
    //tx and ty let the dealerSister find the junkie
    fillWithTrees(x, y, w, h, 2);
  }
  
  //this handles the interactions with the dealer's sister
  public void dealerSisterTalk(int i){
    if (i==0){
      getResponse(
                  "You talk to a woman. She mentions how her sister was recently killed by a crazy\n" +
      ...
      if (junkieState == UNAWARE){
        junkieState = AWARE;
        junkie.place(junkie.x, junkie.y);
      }
    }
    else if (i==1){
      getResponse("You present the killer's head. The woman doesn't know what to do with it.\n" +
                  "She has you throw it away, but thanks you sincerely and presents your reward:\n" +
                  "A colorful piece of stone." + (knowAboutGoal ? " A shard of the Widow's Heart." : ""));
      receivedRewardForJunkie = true;
      tibble.obtainShard();
      //the junkie is gone now, so remove his reference to call the Garbage Collector
      junkie = null;
    }
    else if (i==2){
      getResponse("You talk to the woman whose sister was killed. She thanks you again for your help.");
    }
    else if (i==3){
      getResponse("This is the woman whose sister was killed by the addict.",
                  new DialogButton("Tell her you found the killer's camp"){
        public void press(){
          dealerSisterTalk(4);
        }
      },
                  new DialogButton("Don't mention it")
                 );
    }
    else if (i==4){
      getResponse("You describe the location of the killer's camp. The woman thanks you sincerely.");
      dealerSister.intention = SolidActor.MOVE;
      dealerSister.tx = junkie.x;
      dealerSister.ty = junkie.y;
      ...
      getResponse("The camp is abandoned. The woman studies the area, looking for tracks.");
    }
  }
  ...
  public static Fencer junkie;
  public static FirePit fip;
  public static int junkieState = UNAWARE;
  
  public void makeJunkieCamp(int x, int y){
    int w = 30;
    int h = 30;
    for (int i=x; i<x+w; i++){
      for (int j=y; j<y+h; j++){
    ...
    //the junkie
    junkie = new Fencer(SolidActor.IMMEDIATE_ATTACK, player){
      ...
      public void reset(){
        if (junkie.action == DIE && junkieState == CHOOSING){
          //You just killed the junkie!
          junkieState = KILLED;
          Game.getResponse("You kill the addict. It's ugly. He's crying, bleeding, trying to hold in his guts.\n" +
                           "When it's over, you take his head.");
        }
        super.reset();
      }
      public ScaledImage draw(){
        if (junkieState == AWARE && distanceBetween2(this.x, this.y, target.x, target.y) < 5*5){
          //you got close enough. Time for the discussion!
          junkieState = CHOOSING;
          junkieTalk(0);
        }
        return super.draw();
      }
    ...
    junkie.behavior = Navigator.DEFEND_TERRITORY;
    //set junkie's "territory"
    junkie.wx = junkie.tx - 8;
    junkie.wy = junkie.ty - 8;
    junkie.ww = 16;
    junkie.wh = 16;
    junkie.setDetectionRange(4);
    //fill with trees
    fillWithTrees(x, y, w, h, 2);
  ...
  //this handles dialog with the junkie
  public void junkieTalk(int i){
    if (i == 0){
      getResponse("You find a man matching the description of the addict who killed\n" +
                  "the woman's sister. He is clearly anxious.", 
                  new DialogButton("Confront him about the murder"){
        public void press(){
          junkieTalk(1);
        }
      },
                  new DialogButton("Leave"){
                    public void press(){
                      junkieTalk(2);
                    }
                  }
      ...
      getResponse("'It was an accident! I did not mean to kill her! But she attacked\n" +
                  "me and I had to defend myself. Please don't kill me!'",
                  new DialogButton("'Why did she attack you?'"){
        public void press(){
          junkieTalk(3);
        }
      },
      ...
      getResponse("The man watches you carefully.");
    }
    else if (i == 3){
      getResponse("'I think she was afraid I would hurt her and she tried to strike first.'",
                  new DialogButton("'Why did she think you would hurt her?'"){
        public void press(){
          junkieTalk(4);
        }
      },
      ...
      getResponse("'I had drawn my sword.'",
                  new DialogButton("'Why?'"){
        public void press(){
          junkieTalk(5);
        }
      },
      ...
      getResponse("'I just needed another hit. I was desperate.'",
                  new DialogButton("'So you tried to rob her?'"){
        public void press(){
          junkieTalk(6);
        }
      },
      ...
      getResponse("'She raised her prices. I could not afford it.\nI would have paid the old price.'");
    }
  }
  
  /**
  This makes a small village. The villagers will
  be celebrating. You can meet a beautiful person and go someplace private with them.
  
  Then they are attacked by raiders, who will target you and any of the villagers.
  
  The raiders are made at the north/west edges, then hidden. They only appear when
  "summoned" by lover.
  **/
  ...
  public void makeFestival(int x, int y){
    ...
    villagers.add(new FestiveVillager(player, "This person is telling a story. You don't want to interrupt.", b.x+5, b.y+1, Math.PI));
    ...
    //Your lover
    villagers.add(new Lover(x+22, y+8.5, Math.PI*0.6, x+19, y+11.5, cabin1.x-(cabin1.width*0.5), cabin1.y-(cabin1.height*0.5),
              cabin1.width, cabin1.height));
    ...
    class FestivalRaider extends Fencer{
      ...
      public void act(){
        if (intention == WANDER || !canHit(target) || !actors.contains(target)){
          /**
           If current target is dead or too far, find the closest acceptable one.
           If no target can be found, begin wandering.
           **/
          Solid newTarget = getClosestSolidInCollection(this, villagers, detectionRange);
          if (newTarget != this){
            //found a new target
            target = newTarget;
            if (intention != ATTACK){
    ...
    //raiders start along the north and west edges of the village
    for (int i=0; i<num; i++){
      //alternate making one along the north and west edge
      raiders.add(new FestivalRaider(villagers.get(i), 
                                     (i % 2 == 0 ? x + 1 + (0.75*i) : x + 1 ),
  ...
  /**
  This makes a castle, inhabited by squatters.
  It contains one special squatter, the "summoner." When you encounter
  him, he will beg for his life. If you spare him, he will flee
  to get reinforcements.
  
  Northwest corner and dimensions (x, y, w, h):
    Entire castle, including walls: x+1, y, k + 30 - x, 40.4
    courtyard: x+2, y+1, k-x-2.3, 38.4
    infirmary: k+0.3, y+1, 10.4, 10.4
    lounge: k+11.3, y+1, 8.4, 10.4
    storage/workshop: k+20.3, y+1, 9.7, 10.4
    great hall: k+2.6, y+14.3, 13.1, 12.8
    kitchen: k+16.3, y+18.7, 11.4, 8.4
    pantry: k+16.3, y+14.3, 11.4, 3.8
    common bedroom 1: k+0.3, y+30, 11.4, 10.5
    common bedroom 2: k+12.3, y+30, 11.4, 10.5
    master bedroom: k+24.3, y+30, 5.7, 10.5
  ...
  **/
  public void makeCastle(int x, int y){
    ...
    final Fencer gardener = new Fencer(player, x+4+(spacing*2.5), y+8 + 
                                       (Math.random() < 0.5 ? 0 : 24.4)){
      protected boolean canDetect(Solid s){
        //is the target within the "visibility" area?
        if (isVisible()){
          //yes; are they within normal detection range?
    ...
    //summoner
    final Fencer summoner = new Fencer(player){
      ...
      public void act(){
        ...
        if (behavior == Navigator.STAND){
          ...
          if (intention == SolidActor.STAND){
            ...
            if (isVisible() && distanceBetween2(player.x, player.y, this.x, this.y) < 4*4){
              //if very close, talk
              hiding = false;
              getResponse("'Whoa, whoa, whoa! Please, don't kill me!'",
                          new DialogButton("Spare him"){
                public void press(){
                  summon(); //call the reinforcements!
                }
              },
                          new DialogButton("No mercy"){
                            public void press(){
      ...
      public void intendToAttack(){
        if (sayThing){
          ...
          getResponse("You see a familiar face: the man whose life you spared.\nHe does not seem eager to return the favor.");
        }
        else {
          //if this happens, either you chose "no mercy" or attacked his reinforcements before he got there
          behavior = STAND_UNTIL_DETECT_THEN_ATTACK;
        }
      ...
      public void interact(){
        if (intention == MOVE){
          getResponse("'Please let me go!'");
        }
      }
    };
    summoner.behavior = Navigator.STAND;
    summoner.intention = SolidActor.STAND;
    summoner.hiding = true;
    summoner.setVisibilityArea(vx, vy, vw, vh);
    summoner.place(k+28.75, y+38);
    summoner.slowSpeed = 0.1;
    summoner.speed = summoner.slowSpeed;
    summoner.setDetectionRange(4.5);
    //(tx,ty) is the point to which the summoner goes after summoning
    summoner.tx = k-8;
    summoner.ty = y+20.7;
    
    //workshop/storage
    new Shelf(k+25.15, y+1.5, 9.7, 1){
      public void interact(){
        getResponse("These shelves have various tools and nails and such,\n" +
  ...
  /**
   This makes a sparring partner with whom you can train. You can fight them,
   but no one gets hurt.
   
   When you first meet him, he kills three attackers
   **/
  public void makeTrainer(int x, int y){
    class Trainer extends Fencer{
      //calmingDown is used to make this "calm down" while resetting.
      boolean calmingDown = false;
      DialogButton canYouTeachMe = new DialogButton("'Can you teach me to fight like that?'"){
        public void press(){
          if (x == tx && y == ty){
            if (haveSword){
            ...
            getResponse("'Meet me at my house. I live east of here.'");
            behavior = STAND;
          }
        }
      };
      public Trainer(double xx, double yy, double wx, double wy, double ww, double wh){
        super(player, xx, yy);
        this.wx = wx;
        this.wy = wy;
        this.ww = ww;
        ...
        behavior = STAND_UNTIL_DETECT_THEN_ATTACK;
        images = getBallAndStick(new Color(90, 90, 30));
        setDetectionRange(0);
        slowSpeed = 0.1;
      }
      public void act(){
        /**
        If behavior == STAND:
          If intention == STAND, he is waiting around for you to talk to him
          If intention == MOVE, he is moving away to make space to talk to you
          If intention == ATTACK, he is fighting you
        If behavior == STAND_UNTIL_DETECT_THEN_ATTACK, he is fighting his attackers
        **/
        if (behavior == STAND){
          if (intention == STAND){
            //Is he at his home?
            if (this.x != tx || this.y != ty){
              ...
              //Yes; face you if you're close enough
              if (distanceBetween2(this.x, this.y, player.x, player.y) < 5*5){
                direction = Angle.get(player.x-this.x, player.y-this.y);
              }
              else {
            ...
            double dis2 = distanceBetween2(this.x, this.y, player.x, player.y);
            if (dis2 > 6*6){
              //Too far! Just return to home.
              intention = STAND;
            }
            else if (x >= wx && x < wx+ww && y >= wy && y < wy+wh && dis2 < 3*3){
              //Too close; back up
              moveAwayFrom(player.x, player.y);
              direction = Angle.get(player.x - this.x, player.y - this.y);
            }
            else if (dis2 >= 3*3){
              //Time to fight!
              intention = ATTACK;
              target = player;
              //Choose a random fighting style
              double r = Math.random();
              if (r < 1.0/3){
          ...
          else if (intention == ATTACK){
            if (distanceBetween2(player.x, player.y, this.x, this.y) > 6*6){
              //too far! Just return home.
              intention = STAND;
              resetActions();
              state = 0;
            }
            else if (!calmingDown){
              //don't keep fighting if already hit
              super.act();
            }
          }
        }
          ...
          if (enemies.size() == 0){
            ...
            if (state > 0){
              ...
              //done fighting
              if (distanceBetween2(this.x, this.y, player.x, player.y) < 5*5){
                //You are too close; he will watch you
                direction = Angle.get(player.x-this.x, player.y-this.y);
              }
              else {
                //You are far enough; he will leave
                behavior = STAND;
              }
            }
          }
            ...
            if (state > 0){
              //Already doing something; continue it
            }
            else if (detectionRange > 0){
              //Find the closest living enemy to attack
              target = getClosestSolidInCollection(this, enemies, detectionRange);
              //Set fightingStyle to be the one that beats target
              Fencer f = (Fencer) (target);
      ...
      public ScaledImage draw(){
        if (behavior == STAND_UNTIL_DETECT_THEN_ATTACK && enemies.size() > 0){
          if (detectionRange == 0 && distanceBetween2(this.x, this.y, player.x, player.y) < 4.5*4.5){
            //Begin the fighting!
            setDetectionRange(6);
            for (Solid enemy : enemies){
              Fencer f = (Fencer) (enemy);
              f.setDetectionRange(6);
            }
            getResponse("You witness one man fighting three.");
          }
        }
        return super.draw();
      }
      public void interact(){
        if (behavior == STAND_UNTIL_DETECT_THEN_ATTACK || (x == tx && y == ty)){
          getResponse("You talk to the man who just killed three opponents.",
                      new DialogButton("'Why did they attack you?'"){
            public void press(){
              getResponse("'They wanted to rob me. This area is dangerous.'", canYouTeachMe, new DialogButton("Ok"));
            }
          }, canYouTeachMe, new DialogButton("Ok"));
        }
        else if (intention == MOVE){
          getResponse("'Back up a bit.'");
        }
      }
      public int getDamage(){
        //Return -1 when attacking player, to avoid actual damage
        if (target == player){
          //give a training message
          player.resetActions();
          player.state = 0;
          switch (fightingStyle){
            case IMMEDIATE_ATTACK:
              getResponse("'Got you! When I attack, press r to parry and riposte. You have to time it.'");
              break;
            case COUNTERATTACK:
              getResponse("'Got you! If I try to counter, press f to fake me out with a feint, then attack.'");
              break;
            case FEINT_THEN_ATTACK:
              getResponse("'Got you! If I try to fake you out with feints, don't try to counter the feints.\n" +
                          "Instead, back up to draw me in, then step forward and attack as I feint.'");
              break;
          }
          calmingDown = true;
          resetters.add(this);
      ...
      public void hit(int damage){
        if (damage == 0){
          //Was hit by player
          if (behavior == STAND && intention == ATTACK){
            ...
            getResponse("To talk with this man, press space when not in combat mode.");
          }
        }
      }
      public void reset(){
    ...
    final Trainer trainer = new Trainer(x+5, y+5, x+20, y+5, 10, 10);
    ShingledRoof shin = new ShingledRoof(5, 5){
      public void hit(int damage){
        getResponse("Don't hit this guy's house. Don't be a jerk.");
      }
    };
    moveIfCan(shin, x+30, y+10);
    class TrainerEnemy extends Fencer{
      public TrainerEnemy(double xx, double yy, int fightingStyle){
        super(fightingStyle, trainer, xx, yy);
        setDetectionRange(0);
      }
      public void die(){
        trainer.enemies.remove(this);
        super.die();
      }
    }
    ArrayList <TrainerEnemy> trainerEnemies = new ArrayList <TrainerEnemy> (3);
    trainerEnemies.add(new TrainerEnemy(trainer.x+5, trainer.y, Fencer.IMMEDIATE_ATTACK));
    trainerEnemies.add(new TrainerEnemy(trainer.x+6, trainer.y+1, Fencer.FEINT_THEN_ATTACK));
    trainerEnemies.add(new TrainerEnemy(trainer.x+7, trainer.y-1, Fencer.COUNTERATTACK));
    trainer.enemies = trainerEnemies;
  }
  
  /**
  The stalkingArea is a larger area with a smaller "center" area within.
  While you explore the larger area, enemies will "stalk" you, remaining
  just on the edge of the screen, but not engaging unless cornered.
  
  As you approach the center, someone will approach you and talk. Then,
  you must fight.
  ...
  **/
  ...
  public void makeStalkingArea(int x, int y){
    ...
    class Stalker extends Fencer{
      public Stalker(Solid target, double x, double y){
        super(target, x, y);
        wx = areax;
        wy = areay;
        ww = w;
    ...
    for (int i=0; i<2; i++){
      for (int j=0; j<2; j++){
        Stalker s = new Stalker(player, x + 1 + ((w-2)*i), y + 1 + ((h-2)*j));
        stalkers.add(s);
        s.friends = stalkers;
      }
    }
    //the stalkerTalker is the stalker that talks to you..
    Fencer stalkerTalker = new Fencer(player, x + (w/2), y + (h/2)){
      boolean spoke = false; //Whether he has spoken to you yet
      public void act(){
        if (intention == STAND && target.x >= vx && target.x < vx+vw && target.y >= vy && target.y < vy+vh){
          //If you enter the area he "guards," he will approach
          intendToAttack();
        }
        if (!spoke && distanceBetween2(this.x, this.y, target.x, target.y) < 3.5*3.5){
          getResponse("'Hello! I see you have discovered our little growing operation.\n" +
    ...
    stalkerTalker.setDetectionRange(1);
  ...
  public void actionPerformed(ActionEvent e){
    ...
    if (narration){
      System.out.println("acting:    "+(t2-t1));
      System.out.println("drawing:   "+(t3-t2));
      System.out.println("resetting: "+(t4-t3));
      narration = false;
    }
  }
  **/
  ...
  /**
  Beware of using the following keys: z, c, n, a, s, e, y, u, i, o
  l also seems bad
  They can cause Java to stop receiving key events!
  This issue can apparently be resolved via a command in the Terminal,
  but requiring that means requiring other users to do something tedious
  **/
  ...
  public void keyPressed(KeyEvent e){
    ...
    if (key == KeyEvent.VK_C && haveSword){
      toggleCombatMode();
    }
    //used for testing.
    ...
    if (key == KeyEvent.VK_P){
      getResponse("Game paused.", resumeButton);
    }
  ...
  public static void drawScreen(){
    ...
    //Then it gets rotated to make visibleImage, which is actually drawn to the screen
    ...
    for (int i=xmin; i<xmax; i++){
      for (int j=ymin; j<ymax; j++){
        ...
        imageMover.setToTranslation((frameWidth075)+((i-x)*tileWidth), (frameHeight075)+((j-y)*tileHeight));
    ...
    for (Solid s : relevantSolids){
      ...
      pre-rotation scaling:
      scale by (swc * tileWidth / iW) x (shc * tileHeight / iH)
      This makes the image the size of a tile if swc=shc=1, or 
      something else, according to the pre-rotation scaling coefficients
      image center at (swc*tileWidth*0.5, shc*tileHeight*0.5)
      
      rotation:
      if (direction != 0),
      rotate by (-s.direction) around point (swc*0.5*tileWidth, shc*0.5*tileHeight)
      image center at (swc*tileWidth*0.5, shc*tileHeight*0.5) (unchanged)
      
      post-rotation scaling:
      now scale by width x height
      This makes the image fill the appopriate number of tiles
      image center at (width*swc*tileWidth*0.5, height*shc*tileHeight*0.5)
      
      translation:
      the image's center should be at:
      (frameWidth075 + ((s.x-x) * tileWidth), frameHeight075 + ((s.y-y) * tileHeight))
      Therefore, translation is that, minus the actual center:
      (frameWidth075 + ((s.x-x) * tileWidth) - (width*swc*tileWidth*0.5),
       frameHeight075 + ((s.y-y) * tileHeight) - (height*shc*tileHeight*0.5))
      Factor out tileWidth/Height:
      (frameWidth075 + (tileWidth * (s.x - x - (width*swc*0.5))),
       frameHeight075 + (tileHeight * (s.y - y - (height*shc*0.5))))
      **/
