...
public abstract class Solid extends Obstacle{
  ...
  public Solid(double width, double height, int shape){
    /**
    note that the constructor does not ask for x/y values.
    When a solid is placed, appropriate tiles should be connected.
    Do this with moveTo
    **/
    this.width = width;
    this.height = height;
  ...
  public void connectTiles(Collection <Tile> intersectingTiles){
    if (tiles.size() > 0){
      JOptionPane.showMessageDialog(null, "Warning! Attempting to connect tiles to a solid with a non-empty tiles Collection!");
    }
  ...
  //Note: moveTo should be accompanied by disconnecting and reconnecting relevant tiles!
  ...
  //SolidActors that react to their opponents use this to determine their own action
  ...
  /**
  Every Solid has a "draw" method that returns a ScaledImage to be drawn.
  Some Solids are "hidden," meaning they will not be visible unless the
  player is within a certain "visibility area," designated by vx,vy
  and vw,vh.
