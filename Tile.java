...
public class Tile extends Obstacle implements Serializable{
	...
	static BufferedImage[][] riverImages = ImageMaker.getRiver("horizontalRiver.png");

	// stone tiles should "fit" together, so they're all chopped from a single,
	// larger picture
	...
	/**
	 * a Tile's constructor creates an "undesignated" tile Use setType to actually
	...
	public BufferedImage draw() {
		...
		if (type == RIVER) {
			/**
			 * Rivers are special because they are animated. The game tracks a special
			 * "riverIndex" variable Each river tile draws an image from an array. Its image
			 * is image number (riverIndex + cx), where x is its x-value and c is a
			 * constant.
			 * 
			 * Note that rivers are assumed to run horizontally
			 **/
			return riverImages[((x * 50) + RiverAnimator.index) % riverImages.length][y % 2];
		} else if (image == null) {
			switch (type) {
			case FOREST_FLOOR:
