...
public class Player extends SolidActor {
	...
	public transient BufferedImage[] images = null;
	...
	/**
	 * If this is false, player will not "act" when its act method is called, except
	 * to heal. Use this when the Game should "take over" player's movement.
	 **/
	...
	/**
	 * Player's attack is not unparriable in the way others are. Instead, when
	 * attacking, and the target has already begun parrying, they do a "bad parry"
	 * afterward, which does not stop the attack
	 **/
	...
	/**
	 * Player has no Behavior, because its actions are handled directly in the act
	 * method. When (state == 0), Player is ready to choose a new action. When a key
	 * is pressed, this changes newAction, to be called the next time state is zero.
	 **/
	public void act() {
		...
		/**
		 * For movement, a movement "vector" is constructed, based on which keys are
		 * down. If two opposite movement keys are down, they cancel out. If you try to
		 * move diagonally, the total movement will be scaled such that you still travel
		 * the same distance.
		 **/
		...
		/**
		 * When an action finishes, the state returns to zero, but the current action is
		 * still not "NULL." At this point, if the action is not NULL, it changes to
		 * NULL. If it is NULL, then the next action is drawn from nextAction. This
		 * takes a step, which puts a one-step delay between ending an action and
	...
	public ScaledImage draw() {
		if (images == null) {
			images = Fencer.getBallAndStick(Color.DARK_GRAY);
		}
		switch (action) {
		case ATTACK:
	...
	public void hit(int damage) {
		...
		if (health == maxHealth) {
			...
			Game.timer.stop();
			Game.getResponses("You died.", new Game.DialogButton("Ok") {
				public void press() {
					System.exit(0);
				}
			});
