...
public class Lover extends Navigator{
  BufferedImage image = Fencer.getBallAndStick(new Color(120, 32, 32))[0];
  ...
  final private Game.DialogButton goSomePlacePrivate =
    new Game.DialogButton("'Shall we go some place private to talk?'"){
    public void press(){
  ...
  public Lover(double x, double y, double direction, double tx, double ty, double wx, double wy, double ww, double wh){
    super(1, 1);
    place(x, y);
    ...
    if (intention == STAND){
      if (!beginDance){
        ...
        Game.getResponse("You meet a beautiful stranger named Alex.",
                         new Game.DialogButton("Ask Alex to dance"){public void press(){dance();}},
      ...
      if (action == STAND){
        ...
        Game.getResponse("You and Alex talk for some time.",
                         goSomePlacePrivate,
                         new Game.DialogButton("Ok"));
      }
      else if (action == KISS){
